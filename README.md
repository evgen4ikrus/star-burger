...
Сайт можно посмотреть по ссылке - [my-burger.tk](https://my-burger.tk/).
## Как запустить dev-версию сайта
<details>
<summary>C помощью Docker</summary>
...
- `ALLOWED_HOSTS` — [см. документацию Django](https://docs.djangoproject.com/en/3.1/ref/settings/#allowed-hosts). По умолчанию - `0.0.0.0:8000`.
...
- `DATABASE_URL` — url для подключения к базе данных. По умолчанию - `postgres://admin:12345@db:5432/burger`
- `POSTGRES_DB` - название БД. По умолчанию 'bugrer'
- `POSTGRES_USER` - имя пользователя БД. По умолчанию 'admin'
- `POSTGRES_PASSWORD` - пароль пользователя БД. По умолчанию '12345'

Создайте образы и запустите контейнеры:
```sh
docker-compose up --build
```
Откройте сайт в браузере по адресу [http://0.0.0.0:8000/](http://0.0.0.0:8000/).
...
Определите переменные окружения `SECRET_KEY` и `YANDEX_API_KEY` (можно получить в [кабинете разработчика](https://developer.tech.yandex.ru/services/)). Создать файл `.env` в каталоге `star_burger/backend` и положите туда такой код:
```sh
...
Перейдите в каталог `frontend` проекта и установите пакеты Node.js:
